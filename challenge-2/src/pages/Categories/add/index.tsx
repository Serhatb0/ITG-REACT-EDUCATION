...
function CategoryAdd() {
  ...
  const addCategoryMutation = useMutation(
    (newOrder:any) => axiosInstance.post('categories', newOrder),
    {
      onSuccess: () => {
        reset(); 
      },
      onError: (error) => {
        console.error('Add order mutation error:', error)
      },
    }
  );
...
const handleAddCategory = async (newCategory:any) => {
  ...
  return (
    ...
    >
      {addCategoryMutation.isLoading ? <CircularProgress size={24} /> : "Submit"}
    </Button>
    </form>
  )
}
